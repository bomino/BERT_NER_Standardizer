...
def load_labeled_data(file_path: str):
    ...
    Returns:
        list: List of dictionaries containing tokens and NER tags.
    """
...
# Load data from file
data_file = "output01.json"  # Change to "labeled_data.csv" if using CSV
...
# Oversample rare labels (labels with count < 20 for more aggressive oversampling)
rare_labels = [label for label, count in label_counts.items() if count < 20]
if rare_labels:
    rare_examples = [item for item in train_data if any(label in rare_labels for label in item["ner_tags"])]
    train_data.extend(rare_examples * 2)  # Duplicate rare examples twice
    logger.info(f"Oversampled {len(rare_examples) * 2} examples with rare labels.")
...
# Tokenize and align labels (label all subwords)
def tokenize_and_align_labels(examples):
    """
    Tokenize inputs and align labels with tokenized subwords.
    
    Args:
        examples (dict): Dictionary containing tokens and NER tags.
...
# Post-processing function to merge subwords
def merge_subwords(predictions):
    """
    Merge subword tokens in NER predictions.
    
    Args:
        predictions (list): List of dictionaries containing entity predictions.
    
    Returns:
        list: List of merged entity predictions.
    """
    merged_entities = []
    current_entity = None
    for entity in predictions:
        if entity['word'].startswith("##"):
            if current_entity:
                ...
                # If no current entity, treat it as a standalone word
...
# Test the model on a sample with post-processing
from transformers import pipeline
ner_pipeline = pipeline("ner", model="bert_construction_ner", tokenizer="bert_construction_ner", 
                        aggregation_strategy="simple")
test_description = "ITEM # 476043 TAPCON 1/4-IN X 7-IN SDS DRILLBIT"
result = ner_pipeline(test_description)

# Apply post-processing to merge subwords
merged_result = merge_subwords(result)

print("Test Prediction (with merged subwords):")
for entity in merged_result:
